...
class RWLockfile {
    ...
    unlock(type) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (!type) {
                yield this.unlock('read');
                yield this.unlock('write');
                return;
            }
            if (!this.count[type])
                return;
            yield this._removeJob(type);
            this._count[type] = 0;
        });
    }
    unlockSync(type) {
    ...
    check(type) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const f = yield this._fetchFile();
            const status = this._statusFromFile(type, f);
            if (status.status === 'open')
                return status;
            else if (status.status === 'write_lock') {
                if (!(yield isActive(status.job.pid))) {
                    this.debug(`removing inactive write pid: ${status.job.pid}`);
                    delete f.writer;
                    yield this.writeFile(f);
                    return this.check(type);
                }
                return status;
            }
            else if (status.status === 'read_lock') {
                const pids = yield Promise.all(status.jobs.map((j) => tslib_1.__awaiter(this, void 0, void 0, function* () {
                    if (!(yield isActive(j.pid)))
                        return j.pid;
                })));
    ...
    _fetchFile() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                let f = yield this.fs.readJSON(this.file);
    ...
    _lock(type, opts) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            while (true) {
                try {
                    yield this.tryLock(type, opts.reason);
                    return;
                }
                catch (err) {
                    if (err.code !== 'ELOCK')
                        throw err;
                    yield ifLockedCb(err.status);
                    if (opts.timeout < 0)
                        throw err;
                    // try again
    ...
    tryLock(type, reason) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const status = yield this.check(type);
            if (status.status !== 'open') {
    ...
    writeFile(f) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (!f.writer && !f.readers.length) {
                yield this.fs.remove(this.file);
            }
            else {
                yield this.fs.outputJSON(this.file, this._stringifyFile(f));
            }
        });
    }
    writeFileSync(f) {
...
function isActive(pid) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        try {
            return yield isProcessActive.isActive(pid);
        }
        catch (err) {
