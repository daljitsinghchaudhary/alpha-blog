...
class Lockfile {
    ...
    unlock() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            yield this.fs.remove(this.dirPath);
            yield this.fs.remove(this._infoPath);
            this._stopLocking();
        });
    }
    /**
    ...
    check() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const mtime = yield this.fetchMtime();
    ...
    fetchReason() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                const b = yield this.fs.readJSON(this._infoPath);
                return b.reason;
            }
            catch (err) {
                if (err.code !== 'ENOENT')
    ...
    _saveReason(reason) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                yield this.fs.writeJSON(this._infoPath, {
                    version,
                    uuid: this.uuid,
                    pid: process.pid,
                    reason,
    ...
    fetchMtime() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                const { mtime } = yield this.fs.stat(this.dirPath);
                return mtime;
            }
            catch (err) {
                if (err.code !== 'ENOENT')
    ...
    _add(opts) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            yield this._lock(Object.assign({ timeout: this.timeout, retryInterval: this.retryInterval, ifLocked: ({ reason: _ }) => { } }, opts));
            yield this._saveReason(opts.reason);
            this.startLocking();
        });
    }
    _lock(opts) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            yield this.fs.mkdirp(path.dirname(this.dirPath));
            try {
                yield this.fs.mkdir(this.dirPath);
            }
            catch (err) {
                if (!['EEXIST', 'EPERM'].includes(err.code))
                    throw err;
                // grab reason
                const reason = yield this.fetchReason();
                this.debug('waiting for lock', reason, this.dirPath);
                // run callback
                yield opts.ifLocked({ reason });
                // check if timed out
                if (opts.timeout < 0)
                ...
                switch (status) {
                    case 'stale':
                        try {
                            yield this.fs.rmdir(this.dirPath);
                        }
                        catch (err) {
                            if (err.code !== 'ENOENT')
                                throw err;
